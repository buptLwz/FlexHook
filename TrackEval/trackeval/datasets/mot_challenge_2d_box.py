...
class MotChallenge2DBox(_BaseDataset):
        ...
        if self.config["SEQ_INFO"]:
            ...
            with open(seqmap_file) as fp:
                ...
                for i, row in enumerate(reader):
                    ...
                    if ('MOT' in seq) and ('+' not in row[0]):
                        ini_file = os.path.join(self.gt_fol, seq, 'seqinfo.ini')
                        if not os.path.isfile(ini_file):
                            raise TrackEvalException('ini file does not exist: ' + seq + '/' + os.path.basename(ini_file))
                        ini_data = configparser.ConfigParser()
                        ...
                        if seq.startswith('MOT17'):
                            img_path = os.path.join('./datasets/LaMOT-main/sequences/val/MOT17-sequences', seq,'img1')
                        elif seq.startswith('v'):
                            img_path = os.path.join('./datasets/LaMOT-main/sequences/val/SportsMOT-sequences', seq,'img1')
                        elif seq.startswith('uav'):
                            img_path = os.path.join('./datasets/LaMOT-main/sequences/val/VisDrone-sequences', seq)
                        elif '/' in seq:
                            
                            img_path = json.load(open('./datasets/LaMOT-main/sequences/val/TAO_framespervideo.json','r'))[seq]

                        elif seq.startswith('dancetrack'):
                            img_path = os.path.join('./datasets/refer-dance/DanceTrack/training/image_02',seq)
                        else:
                            img_path = os.path.join('./datasets/refer-kitti/KITTI/training/image_02', seq)
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        # if '/' in seq:# or True:
        # 
        #     current_time_keys = list(map(str,frameidpervideo[seq.split('+')[0]]))
     
            # current_time_keys = list(map(str,frameidpervideo['AVA/'+seq.split('+')[0]]))
        # else:
