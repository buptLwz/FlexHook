...
def apply_rotary_emb(xq: torch.Tensor, xk: torch.Tensor, freqs_cis: torch.Tensor):
    xq_0 = xq.float().reshape(*xq.shape[:-1], -1, 2)
    xk_0= xk.float().reshape(*xq.shape[:-1], -1, 2)
    #print(xq_0.shape)

    xq_ = torch.view_as_complex(xq_0)#.copy_()
    #print(xq_.shape)

    xk_ = torch.view_as_complex(xk_0)#.copy_()
...
class RoPEAttention(Attention):
    ...
    def forward(self, x, freqs_cis):
        ...
        qq = q[:, :, 1:].clone()
        kk =  k[:, :, 1:].clone()
        qq, kk = apply_rotary_emb(qq, kk, freqs_cis=freqs_cis)
        q[:, :, 1:]=qq
        k[:, :, 1:]=kk
...
class rope_vit_models(vit_models):
    def __init__(self, rope_theta=100.0, rope_mixed=False, use_ape=False,
        ...
        if self.rope_mixed:
            ...
            t_x, t_y = init_t_xy(end_x = img_size[1] // patch_size, end_y = img_size[0] // patch_size)
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)
        #y =x[:,1:]
        x_out = rearrange(x[:,1:],'B (H W) C->B C H W ',H=14,W=42)
        #x = x[:, 0]
        #x_out = x_out + x[:,0].unsqueeze(-1).unsqueeze(-1)
