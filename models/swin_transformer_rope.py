...
class SimpleSwinTransformer(SwinTransformer):
...
def hf_checkpoint_load(model_name):
    ...
    ckpt_path =f'{model_name}/pytorch_model.bin'
...
def swin_rope_mixed_tiny_patch4_window7_224(pretrained=False, img_size=224):
    if isinstance(img_size,int):
        ...
        window_size = img_size[0] // 32

    model = RoPESwinTransformer(
        img_size=img_size, patch_size=4, embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
        window_size=window_size, mlp_ratio=4, rope_theta=10.0, rope_mixed=True, use_rpb=False,ape=True
    )
    if pretrained:
        state_dict = hf_checkpoint_load("pretrained/swin_rope_mixed_tiny_patch4_window7_224")
...
def swin_rope_mixed_small_patch4_window7_224(pretrained=False, img_size=224):
    ...
    if pretrained:
        state_dict = hf_checkpoint_load("pretrained/swin_rope_mixed_small_patch4_window7_224")
        model.load_state_dict(state_dict, strict=False)
    return model
...
def swin_rope_mixed_base_patch4_window7_224(pretrained=False, img_size=224):
    ...
    if pretrained:
        state_dict = hf_checkpoint_load("pretrained/swin_rope_mixed_base_patch4_window7_224")
        model.load_state_dict(state_dict, strict=False)
    return model
...
def swin_rope_axial_tiny_patch4_window7_224(pretrained=False, img_size=224):
    ...
    model = RoPESwinTransformer(
        img_size=img_size, patch_size=4, embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
        window_size=window_size, mlp_ratio=4, rope_theta=50.0, rope_mixed=False, use_rpb=False,ape=True
    )
    if pretrained:
        state_dict = hf_checkpoint_load("pretrained/swin_rope_axial_tiny_patch4_window7_224")
        model.load_state_dict(state_dict, strict=False)
    return model
...
def swin_tiny_patch4_window7_224(pretrained=False, img_size=224):
    if isinstance(img_size,int):
        window_size = img_size // 32
    else:
        window_size = img_size[0] // 32
    model = SimpleSwinTransformer(
        img_size=img_size, patch_size=4, embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
        window_size=window_size, mlp_ratio=4, use_rpb=False,ape=True
    )
    if pretrained:
        state_dict = torch.load('pretrained/swin_tiny_patch4_window7_224_22k.pth')
        model.load_state_dict(state_dict, strict=False)
    return model